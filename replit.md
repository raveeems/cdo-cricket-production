...
### Player Headshot Images (CSK complete + RCB complete + SRH complete — 74 players total)
...
- **Extending**: To add more teams (MI, DC, KKR, etc.), append new entries to `PLAYER_IMAGES` in `utils/playerImages.ts` with the player's API UUID as key and a `require('../assets/images/players/{id}.jpeg')` as value. Add corresponding JPEG to `assets/images/players/`.